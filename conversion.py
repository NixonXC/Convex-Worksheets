...
def create_pdf(header, description, data, no_of_questions, grade, user_input, diff, subject):
    pdf = FPDF('P', 'mm', 'A4')
    pdf.add_page()
    ...
    pdf.multi_cell(0, 10, f"Chapter: {user_input} | Total Questions: {no_of_questions}", 0, 'C')
    pdf.multi_cell(0, 10, f"Difficulty: {diff} | Subject: {subject} | Grade: {grade}", 0, 'C')
    pdf.set_font('DejaVu', '', 12)

    # Calculate the width of the underscores line
    underscore_line = "______________________________________________________________________"
...
def generate_fsheet(header, description, data, type, grade, user_input, subject):
    pdf = FPDF('P', 'mm', 'A4')
    pdf.add_page()
    pdf.set_margins(20, 30, 20)
    current_dir = os.path.dirname(os.path.abspath(__file__))
    ...
    if type == "sheet":
        ...
        type = "E-Notes"
    pdf.multi_cell(0, 10, f"Chapter: {user_input} | Type: {type}", 0, 'C')
    pdf.multi_cell(0, 10, f"Subject: {subject} | Grade: {grade}", 0, 'C')
    pdf.set_font('DejaVu', '', 12)
